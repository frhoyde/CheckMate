...
function task_done(task_parentDiv) {
  ...
  showUnfinishedTaskInArchive();
  showCompletedTaskInArchive();
}
...
firebase.auth().onAuthStateChanged(function (user) {
  var firebaseRef = firebase
    .database()
    .ref("users/" + user.uid + "/archived_task/unfinished/")
...
firebase.auth().onAuthStateChanged(function (user) {
  var firebaseRef = firebase
    .database()
    .ref("users/" + user.uid + "/archived_task/finished/")
    .orderByChild("time"); // need for all task
  if (user) {
    user = firebase.auth().currentUser;
    // Retrieve new tasks as they are added to our database
...
SearchinputBox.onkeyup = (e) => {
  ...
  if (userData) {
    ...
    for (let i = 0; i < allList.length; i++) {
      ...
      allList[i].setAttribute("href", "archive.html#" + task_string);
      //allList[i].task_id += ;
