...
function showUnfinishedTaskInArchive(){
  var taskArray= [];
    show_unfinised_task_container = document.getElementsByClassName(
        "row unfinished"
      )[0];
        ...
        if (user) {
          ...
          firebaseRef.once("value", (snapshot) => {
            ...
            for (var i, i = 0; i < taskArray.length; i++) {
              ...
              three_dot_unarchive_icon.setAttribute("class", "bi bi-journal-arrow-up");
              ...
              three_dot_delete.setAttribute(
                "onclick",
                "task_delete(add_title.childnodes)"
              );
...
function showCompletedTaskInArchive(){
  var taskArray= [];
    show_completed_task_container = document.getElementsByClassName(
        "row finished"
      )[0];
